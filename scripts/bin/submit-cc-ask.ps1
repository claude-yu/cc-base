...
if ([string]::IsNullOrWhiteSpace($question)) {
    Write-Output "用法: /问cc <你的问题>"
    Write-Output "示例: /问cc 你是什么模型"
    exit 1
}

$chatLogWriter = Join-Path (Split-Path -Parent $PSCommandPath) "chat-log-writer.ps1"

$RunId = (Get-Date -Format "yyyyMMdd-HHmmss") + "-cc-ask"
...
if (Test-Path -LiteralPath $chatLogWriter) {
    try {
        & $chatLogWriter -Channel wechat -Direction in -Lifecycle started -RecordType message -Command "问cc" -RunId $RunId -Text $question | Out-Null
    } catch {}
}

$runnerPath = Join-Path $ControllerRoot "bin\cc-ask-runner.ps1"
Start-Process -FilePath "powershell" -ArgumentList @(
    "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", $runnerPath,
    "-RunId", $RunId, "-ControllerRoot", $ControllerRoot
) -WorkingDirectory $ControllerRoot -WindowStyle Hidden

Write-Output "已开始询问 CC，预计需要 1-3 分钟。"
Write-Output "Run ID: $RunId"
Write-Output "完成后会自动发送结果。"
Write-Output "可手动查看: /查看cc $RunId"
