...
$disabledFile = Join-Path $ControllerRoot "auto-callback.disabled"
...
switch -Regex ($action) {
    "^(开|on|1|enable)$" {
        if (Test-Path -LiteralPath $disabledFile) { Remove-Item -LiteralPath $disabledFile -Force }
    ...
    "^(关|off|0|disable)$" {
        ...
        "1" | Set-Content -LiteralPath $disabledFile -Encoding UTF8
        Write-Output "自动回传: 已关闭"
        Write-Output "需要手动 /查看审查 查看结果。"
    }
    default {
        $status = if (Test-Path -LiteralPath $disabledFile) { "关闭" } else { "开启" }
