...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $latest = Get-ChildItem -LiteralPath $RunsRoot -Directory |
        Where-Object { $_.Name -like "*-codex-ask" } |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
    if ($null -eq $latest) {
        Write-Output "No codex-ask run found."
        exit 1
    }
    $RunId = $latest.Name
}
...
$exitCodePath = Join-Path $RunDir "codex-answer.exitcode.txt"
...
function Get-RunningStage {
    ...
    if (Test-Path -LiteralPath $answerPath) { return "codex_running" }
    if (Test-Path -LiteralPath (Join-Path $RunDir "question.md")) { return "prepare" }
    return "prepare"
}
...
function Get-StageText {
    ...
    switch ($Stage) {
        "prepare" { return "准备问题" }
        "codex_running" { return "Codex 思考中" }
...
if ($exitCode -eq "0") {
    ...
    Write-Output "Codex exit: $exitCode"
    Write-Output "Directory: $RunDir"
    Write-Output ""
    Write-Output "Codex answer:"
